...
(function ($) {
  ...
  tag.src = "https://vm.gtimg.cn/tencentvideo/txp/js/iframe/api.js";
  ...
  var youTubeIdReg =
    /(youtube?\.com|youtu\.be)\/(watch\?v=|v\/|u\/|embed\/?)?([\w-]{11})(.*)?/i;
  ...
  function buildfigure(mediafigure, curmedia, totalmedia) {
    ...
    if (mediafigure[1] || totalmedia > 1) {
      figurehtml +=
        '<figcaption class="thecaption"><div class="captioninner">' +
        countermarkup +
        (mediafigure[1] || "") +
        "</div></figcaption>\n";
    }
  ...
  function buildvideocontainer(mediafigure, curmedia, totalmedia) {
    ...
    videohtml +=
      '<div class="jkvideowrapper"><div id="jkvideotemp"></div></div>\n';
    if (mediafigure[1] || totalmedia > 1) {
      videohtml +=
        '<div class="thecaption"><div class="captioninner">' +
        countermarkup +
        (mediafigure[1] || "") +
        "</div></div>\n";
    }
  ...
  var responsivegallery = {
      ...
      $(document).on("keyup", function (e) {
        if (responsivegallery.$galleryui.css("display") == "block") {
          if (e.keyCode == KEYCODE_ESC) {
            ...
            responsivegallery.activegallery.media.length > 1
          )
      ...
      $(window).on("resize.adjustuidimensions", function () {
        if (responsivegallery.$galleryui.css("display") == "block") {
          if (responsivegallery.$galleryfigurearea.hasClass("isvideo")) {
            var captionheight =
              responsivegallery.$galleryinner.find("div.thecaption").height() ||
              0;
            var maxheight =
              responsivegallery.$galleryinner.height() - captionheight;
            var maxwidth = maxheight * videoaspectratio - 30;
            if (maxwidth > 1000) {
              // if video width is past 1000, create some left/right margin inside window
              maxwidth -= 100;
            }
            responsivegallery.$galleryfigurearea.css({ width: maxwidth });
          } else {
            var $galleryimg =
              responsivegallery.$galleryfigurearea.find("img:eq(0)");
            $galleryimg.css(
              "maxHeight",
              responsivegallery.$galleryinner.height() - 10
            );
    ...
    showgallery: function () {
      ...
      $navbuttons.css(
        "display",
        this.activegallery.media.length > 1 ? "block" : "none"
      );
    ...
    populategallery: function () {
      ...
      preloadmedia(media[curmedia][0], function (err) {
        if (err) {
          ...
          responsivegallery.$errordiv
            .css("display", "block")
            .attr("title", "Error loading " + media[curmedia][0]);
          var $gallerymedia =
            responsivegallery.$galleryfigurearea.find("img:eq(0)");
          var $figcaptioninner = responsivegallery.$galleryfigurearea.find(
            "figcaption div.captioninner"
          );
        } else {
          //  load either media or Youtube video
          var isvideo = isYoutubeReg.test(media[curmedia][0]);
          if (isvideo) {
            responsivegallery.$galleryfigurearea.addClass("isvideo");
            responsivegallery.$galleryfigurearea
              .empty()
              .html(
                buildvideocontainer(media[curmedia], curmedia, media.length)
              );
            ...
            responsivegallery.$galleryfigurearea
              .empty()
              .html(buildfigure(media[curmedia], curmedia, media.length));
          }
        }
        ...
        responsivegallery.$galleryfigurearea.find("figure").css({
          transitionDuration:
            responsivegallery.activesettings.fxduration / 1000 + "s",
          opacity: 1,
        });
      });
    },
    ...
    navigate: function (keyword) {
      ...
      if (
        this.activesettings.autoclose &&
        mediashown[mediashown.length - 1] == "all" &&
        targetmedia == startingmedia
      ) {
        this.hidegallery();
        return;
      }
      ...
      if (
        media.length > 1 &&
        mediashown.length >= media.length &&
        this.activesettings.autoclose
      ) {
        mediashown.push("all");
      }
  ...
  $.fn.responsivegallery = function (settings) {
    ...
    return this.each(function (i) {
      ...
      if ($galleryui.length == 0) {
        // build gallery UI only once
